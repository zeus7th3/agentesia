...
def verificar_rag_setup():
    """Verifica si el sistema RAG está configurado correctamente"""
    processor = DocumentProcessor()
    return processor.chroma_path.exists()

def configurar_rag():
    """Configura el sistema RAG"""
    with st.spinner("Configurando el sistema RAG... Esto puede tardar unos minutos."):
        processor = DocumentProcessor()
...
def procesar_consulta(consulta: str, ticket_id: str):
    """Procesa la consulta del usuario a través del grafo de helpdesk"""
    ...
    try:
        ...
        return None, historial_procesamiento, config

def main():
    """Aplicación principal"""
    st.title("HelpDesk 2.0 con RAG + ChromaDB")
    st.markdown("*Sistema inteligente con LangGraph y busqueda vectorial*")
    ...
    with st.sidebar:
        ...
        if rag_configurado:
            ...
            st.warning("RAG no configurado.")
            if st.button("Configurar RAG"):
                if configurar_rag():
                    st.success("RAG configurado correctamente.")
                    st.rerun()
                else:
                    st.error("Error al configurar RAG.")
        ...
        if st.button("Reconfigurar RAG"):
            if configurar_rag():
                st.success("RAG reconfigurado correctamente.")
                st.rerun()
        if st.button("Limpiar Tickets"):
            st.session_state.tickets.clear()
            st.success("Tickets limpiados.")
            st.rerun()

    if not rag_configurado:
        st.warning("El sistema RAG no está configurado. Por favor, configurelo desde el panel lateral.")
        return
        ...
        if not st.session_state.tickets:
            ...
            for ticket_id, ticket_data in reversed(list(st.session_state.tickets.items())):
                with st.expander(f"{ticket_id} - {ticket_data['timestamp']}", expanded=True):
                    ...
                    # Mostrar progresos del procesamiento
                    ...
                    if resultado.get("categoria"):
                        st.markdown(f"**Categoria:** {resultado['categoria']}")
                    ...
                    if resultado.get('confianza',0) > 0:
                        ...
                        progress_color = "green" if confidence >= 0.65 else "orange" if confidence >= 0.45 else "red"
                    ...
                    if resultado.get('requiere_humano') and not resultado.get('respuesta_final'):
                        ...
                        with col_btn2:
                            if st.button(f"Usar RAG", key=f"rag_{ticket_id}"):
                                ...
                                st.success("Resputa RAG aplicada")
                                st.rerun()
